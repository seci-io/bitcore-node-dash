...
To develop bitcore-node-seci:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-seci.git
...
We now will setup symlinks in `bitcore-node-seci` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-seci
ln -s ~/bitcoind-rpc-seci
```
...
cd devnode
mkdir node_modules
touch bitcore-node-seci.json
touch package.json
```

Edit `bitcore-node-seci.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-seci/bin/bitcore-node-seci start
```