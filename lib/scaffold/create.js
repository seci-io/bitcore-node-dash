...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'bitcore-lib-seci': '^' + bitcore.version,
    'bitcore-node-seci': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-seci.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitcore-node-seci directory and configuration
